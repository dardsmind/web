...
<div class="page-content">
	...
	<div class="page-bar">
		<ul class="page-breadcrumb">
			...
			<li>
				<a href="javascript:;">Category list</a>
				<i class="fa fa-circle"></i>
			</li>
		</ul>
	</div>
	<!-- END PAGE BAR -->
	<!-- BEGIN PAGE TITLE-->
	<h3 class="page-title"> Blog categories
	</h3>
	<!-- END PAGE TITLE-->
	<div class="row">
		<div class="col-md-12">
	...
	<div class="modal-content">
	  ...
	  <div class="modal-body">
		...
		<div class="note note-info note-bordered">
			<p>
			Note: Category name has to be one word only otherwise you may put a under score character in case of compound words. example: information_technology
			</p>
		</div>		  
...
$( document ).ready(function() {
    ...
    $('#tblRoles').DataTable({
		...
		info:false,
        serverSide: true,
        ajax: "{{ route('dashboard.category.data') }}",
	...
	$('#frmCategory').on('submit', function(e) {
		...
		$.ajax({
			type: "POST",
			url: "{{ route('dashboard.category.save')}}",
			data: $(this).serialize(),
			beforesend: function(){
				$('#mdRole').modal('hide');
			},
		...
		swal({ title: "Are you sure?",   
				...
				.then((willDelete) => {
				  if (willDelete) {
					...
					$.ajax({  
						url:'{!! route('dashboard.category.delete') !!}', 		
						type:"delete",  
						data: value,  
						success:function(){ 
							$('#tblRoles').DataTable().ajax.reload();
